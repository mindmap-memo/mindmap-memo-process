          ...
          jsonb_build_object(
            ...
            'memos', COALESCE(
              (
                ...
                FROM memos m
                WHERE m.page_id = p.id AND (m.user_id = users.id OR m.user_id IS NULL)
              ),
              '[]'::jsonb
            ),
            'categories', COALESCE(
              (
                ...
                FROM categories c
                WHERE c.page_id = p.id AND (c.user_id = users.id OR c.user_id IS NULL)
              ),
              '[]'::jsonb
            ),
            'quickNavItems', COALESCE(
              (
                ...
                WHERE q.page_id = p.id AND (q.user_id = users.id OR q.user_id IS NULL)
              ),
              '[]'::jsonb
            )
          )
