...
export async function GET() {
  try {
    ...
    const items = await sql`
      SELECT
        id, user_id, type, target_id, page_id, created_at
    ...
    const quickNavItems = items.map((item: any) => ({
      ...
      pageId: item.page_id,
      title: '', // Will be populated from memo/category
    }));
  ...
  try {
    ...
    await sql`
      INSERT INTO quick_nav_items (
        id, user_id, type, target_id, page_id
      )
      VALUES (
        ${quickNavId}, ${session.user.id}, ${type}, ${itemId}, ${pageId}
      )
    `;
