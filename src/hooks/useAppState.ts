...
import { initializeApp, loadPageData } from '../utils/api';
...
export const useAppState = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setIsLoading(true);
        const { pages: loadedPages, quickNavItems } = await initializeApp();

        if (loadedPages.length === 0) {
          // No pages yet, use default
          setPages(DEFAULT_PAGES);
          setCurrentPageId('1');
        } else {
          // Load memos and categories for each page
          const pagesWithData = await Promise.all(
            loadedPages.map(async (page) => {
              const { memos, categories } = await loadPageData(page.id);
              return {
                ...page,
                memos,
                categories,
                quickNavItems: quickNavItems.filter((item) => item.pageId === page.id),
              };
            })
          );

          setPages(pagesWithData);
          setCurrentPageId(pagesWithData[0]?.id || '1');
        }
      } catch (error) {
        console.error('Failed to load data from database:', error);
        // Fallback to localStorage
        const fallbackPages = loadFromStorage(STORAGE_KEYS.PAGES, DEFAULT_PAGES);
        const fallbackPageId = loadFromStorage(STORAGE_KEYS.CURRENT_PAGE_ID, '1');
